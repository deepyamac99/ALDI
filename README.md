...
This repository implements **gradient-based** and **gradient-free** variants of an ALDI (Affine-Invariant / Approximate Langevin Dynamics Inference–type) algorithm to estimate failure probabilities in a dynamical system using ensembles of particles evolved by stochastic dynamics. [web:14]

Both approaches revolve around a **limit state function (LSF)** that defines a failure domain via the long-time behavior of a simple 2D dynamical system, and use ensemble-based updates to concentrate particles near rare failure regions. [web:23]
...
- `aldi_gradient_step(U, T, y, Gamma, dt, k, alpha, eps)`  
  ...
  - Computes:
    ...
    - Covariance-based square-root term `C_half` (using anomalies).  
...
- `aldi_gradient_free_step(U, y, t_grid, Gamma, P0_inv, mu0, dt=0.1, eps=1e-12)`  
  ...
  - Uses:
    - Observation noise inverse `R_inv = Gamma^{-1}`.  
...
Clone the repository and install dependencies using a Python environment (e.g., `venv` or Conda). [web:1][web:32]
...
- Both variants rely on **ensemble statistics** and are suitable for reliability analysis and rare-event estimation in systems defined via black-box forward models. [web:23][web:24]
- Random seeds are set (`np.random.seed(42)`) for reproducible experiments.  
- You can extend the plotting utilities or post-process the saved `.npy` files in separate notebooks for additional diagnostics (e.g., convergence of estimated failure probabilities or comparison between gradient-based and gradient-free behavior). [web:14]
